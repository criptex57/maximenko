...
- **Проекты** — название, описание, обложка, категория, город, площадь, год, задача, решения и галерея;
...
Изображения проектов редактируются через стандартную медиатеку и featured image. Галерея проекта задаётся ID вложений через запятую в блоке «Данные для сайта».
